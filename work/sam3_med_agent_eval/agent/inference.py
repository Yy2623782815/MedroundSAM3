...
# IMPORTANT: import from the local (work) agent package, not the original sam3 repo
...
def run_single_image_inference(
    ...
    debug=False,
    history_text=None,  # NEW: plain text block history
):
    """Run inference on a single image with provided prompt (and optional history_text)"""
    ...
    agent_history, final_output_dict, rendered_final_output = agent_inference(
        image_path,
        text_prompt,
        history_text=history_text,  # NEW: pass through
    ...
    if history_text is not None:
        final_output_dict["history_text"] = history_text  # optional, useful for debugging
