...
def decode_pred_rle_mask(rle_obj, height: int, width: int) -> np.ndarray:
    """
    Decode predicted mask RLE to HxW uint8 mask.
    Supported:
    1) compressed COCO RLE string
    2) COCO RLE dict
    """
...
def union_pred_masks(pred_masks, height: int, width: int) -> np.ndarray:
    """
    Union predicted masks into one HxW uint8 mask.

    Supported inputs:
    1) list[str]            -> compressed RLE list
    2) list[dict]           -> COCO RLE dict list
    3) np.ndarray [N,H,W]   -> binary masks
    4) np.ndarray [H,W]     -> single binary mask
    5) list[np.ndarray]     -> list of binary masks
    """
    ...
    if isinstance(pred_masks, np.ndarray):
        ...
        if arr.ndim == 3:
            ...
            for i in range(arr.shape[0]):
                union_mask = np.logical_or(union_mask, arr[i] > 0)
            return union_mask.astype(np.uint8)
        raise ValueError(f"Unsupported ndarray pred_masks shape: {arr.shape}")
