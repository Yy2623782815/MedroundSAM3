# filename: /root/autodl-tmp/work/medsam3_lora_gt_label_eval/eval_medsam3_lora_gt_label_batch.py
import argparse
import json
import os
import traceback
...
from medsam3_infer import MedSAM3LoRAInferencer
...
def _metric_stats(items, metric_key):
    vals = [
        x[metric_key]
        for x in items
        if x.get("status") == "ok" and x.get(metric_key) is not None
    ]
    if len(vals) == 0:
        return {
...
def summarize_results(records):
    ...
    summary["num_no_pred_mask"] = sum(
        1 for r in records if r.get("no_pred_mask") is True
    )
...
def build_selected_items(dataset_json, split: str, max_samples: int):
    ...
    - if split=all and max_samples > 0:
        use at most max_samples from training and at most max_samples from test
    """
    selected = []
    ...
    with open(results_jsonl_path, "w", encoding="utf-8") as fout:
        for sample_idx, (split_source, item) in enumerate(selected_items):
            ...
            for turn_idx, qobj in enumerate(questions):
                ...
                try:
                    final_outputs = inferencer.predict_single_prompt(
                        image_path=image_path,
                        prompt=prompt,
                    )
                    ...
                    pred_mask = union_pred_masks(
                        final_outputs.get("pred_masks"), height, width
                    )
                    ...
                    num_pred_masks = int(
                        final_outputs.get(
                            "num_detections",
                            len(final_outputs.get("pred_masks", [])),
                        )
                    )
...
def parse_args():
    ...
    parser.add_argument(
        "--load_from_HF",
        action="store_true",
        help="Load base model from Hugging Face instead of local checkpoint_path.",
    )
