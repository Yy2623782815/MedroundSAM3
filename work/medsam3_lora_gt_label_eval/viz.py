# filename: /root/autodl-tmp/work/medsam3_lora_gt_label_eval/viz.py
import numpy as np
...
def _normalize_pred_masks_to_list(pred_masks):
    """
    Normalize pred_masks to list of HxW uint8 masks.
    Supported:
      - np.ndarray [H,W]
      - np.ndarray [N,H,W]
      - list[np.ndarray]
      - RLE/list[dict]/list[str] -> fallback to union only outside this helper
    """
...
def visualize(
    ...
    """
    Visualize prediction masks on original image.

    Behavior:
    - if pred_masks are ndarray masks, render each instance with different color
      and add rank numbers 1,2,3... sorted by pred_scores descending.
    - otherwise, fallback to union mask overlay.
    """
    ...
    # Case 1: instance masks available -> multi-color + numbering
    if len(mask_list) > 0:
        ...
        ranked_scores = [pred_scores[i] for i in indices] if len(pred_scores) == len(mask_list) else [None] * len(mask_list)
    ...
    # Case 2: fallback -> union all masks and overlay with single color
